...
config_path = f"{papka_bota_path}config.txt"
...
beginner_ch = 928941669049577472
role_staff = 803015808242286592
console_ch_18 = 1063535857862201384
...
@bot.command(name='role18')
@commands.has_role(role_helper)
async def print(ctx, *, text):
    channel = bot.get_channel(console_ch_18)
    nick, role = text.split()
    await channel.send(f'lp user {nick} parent add {role}')
    await ctx.reply("Выдал")
    return
...
async def st(ctx, *arg):
    ...
    await ctx.reply(texts_cfg["start_reply"])
    await  asyncio.sleep(2 * 60)
    os.startfile(bat_path)
    await ctx.send('Запустил батник')
    return
...
async def mult(ctx, user : discord.Member):
    ...
    await ctx.send(f'Напиши как зайдёшь')
...
async def mult(ctx, user : discord.Member):
    with open(f"{papka_bota_path}" + "singl.txt", 'r', encoding='utf-8') as data:
        text = data.read()
    await ctx.send(f'{user.mention} привет! \n\n{text}')
    await ctx.send('Напиши как установишь модпак и запустишь мир')
    await ctx.message.delete()
    return
...
async def mult(ctx):
    ...
    await ctx.send(f'Напиши как отправишь заявку')
    await ctx.message.delete()
    return

@bot.command(name='say')
@commands.has_role(role_staff)
async def say(ctx, id, * ,text):
...
async def on_message(message):
    ...
    if message.author.id == 1052275378783256718:
        pass
    elif message.author.id == 1066743268873867375:
        if message.content.startswith('*ping'):
            await message.channel.send('Фыромяу')
    elif message.channel.id == beginner_ch:
        try:
            nick, city, rejim = message.content.split()
        except:
